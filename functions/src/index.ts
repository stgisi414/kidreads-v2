...
export const getTimedTranscript = onRequest(
  ...
  async (request, response) => {
    corsHandler(request, response, async () => {
      ...
      try {
        const ttsUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key=${process.env.API_KEY}`;
        const ttsPayload = {
          model: "gemini-2.5-flash-preview-tts",
        ...
        if (!ttsResponse.ok) {
          throw new Error(`Gemini TTS API failed with status ${ttsResponse.status}`);
        }

        const ttsResult = await ttsResponse.json();
        const audioBase64 = ttsResult?.candidates?.[0]?.content?.parts?.find((part: any) => part.inlineData)?.inlineData?.data;
        ...
        const words = speechResponse.results?.flatMap(result => result.alternatives?.[0].words || []) || [];
        const timedWords = words.map(wordInfo => ({
            word: wordInfo.word,
            startTime: parseFloat(`${wordInfo.startTime?.seconds || 0}.${wordInfo.startTime?.nanos || 0}`),
            endTime: parseFloat(`${wordInfo.endTime?.seconds || 0}.${wordInfo.endTime?.nanos || 0}`),
        }));

        response.status(200).send({ audioContent: audioBase64, timedWords });
      } catch (error) {
        logger.error("Error in getTimedTranscript:", error);
        response.status(500).send("Error generating timed transcript.");
      }
    });
  },
);